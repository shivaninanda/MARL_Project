...
MODE = "computer"
rendezvousCompleted = 0
rendezvousNotCompleted = 0
...
def train_on_one_game(model, rendezvousCompleted, rendezvousNotCompleted, mode = "computer"):
    env = FiveAgents()
    obs = env.reset()
...
#        print(obs)
    ...
    if rendezvous:
            rendezvousCompleted += 1
    else:
        rendezvousNotCompleted += 1
    ...
    if SAVE_MODEL:
        model.save("~/models/model_2000.model")

    print(rendezvousCompleted, rendezvousNotCompleted)
    return model, rendezvousCompleted, rendezvousNotCompleted
...
if __name__=="__main__":
    ...
    if USE_PRETRAINED_MODEL:
        x = keras.models.load_model('~/models/model_250.model')
    else:
        x = make_malicious_agent_network()
        x.summary()

    for game_number in range(250):
        new_x, rendezvousCompleted, rendezvousNotCompleted = train_on_one_game(x, rendezvousCompleted, rendezvousNotCompleted, mode = MODE)
